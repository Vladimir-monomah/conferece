...
function hideListObject(a) {
    ...
    if (id.indexOf('EmptyId') > -1) {
        ...
        $div.html("<br /><span class='deleteObjectNote'>" + jsParams['DeleteObjectMsg'] + ",</span> <a class='action' href='javascript:void(0)' onclick='showListObject(\"" + id + "\");' >" + jsParams['RevertMsg'] + "</a><br /><br />");
        $div.append($hidden);
        $hidden.hide();
    }
    return false;
...
function getMaxEmptyId() {
    if (maxEmptyId == -1) {
        maxEmptyId = jsParams['OrgsCount'];
    }
    return maxEmptyId;
}

function createListObject(withSelect) {
    var $emptyDiv = $('#divEmptyId');
    var $divBefore = $('#divSelectEmptyId');
    if (withSelect == true) {
        $emptyDiv = $divBefore;
    }
