...
<?php
    ...
    Yii::app()->clientScript->registerScript('js.participant.application.params', 'var jsParams=' . CJSON::encode($jsParams).';', CClientScript::POS_HEAD); 
...
<div class="form participant-edit" >  
    ...
    <?php if (($submitAction == 'create') && (Yii::app()->authManager ->isConfAdmin($conf->id) || Yii::app()->authManager ->isAdmin())){ ?>
        <div xclass="note" style="background-color: #F5F5F5;padding:10px;">      
            <?php 
    ...
    <!-- форма участия -->
    <?php
        $data = $conf->participation_types_names(ParticipationType::TYPE_WO_PAPER);
        if (count($data) == 0) {
            echo CHtml::hiddenField('Participant[participation_type]', ParticipationType::AUDIENCE);
        } else {
    ?>
    <div class="row" >
    ...
    <?php foreach($pAttributesEnabled as $i => $attribute) { ?>
        <?php if ($attribute != 'authors') { ?>        
            <?php 
                ...
                if( ($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_DISABLED) && ($attribute != 'report_title')){                    
                    continue; // пропускаем поле
                };
                if($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_MANDATORY_ALL){
                    $htmlOptions['required'] = true;
                };
                if($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_MANDATORY_ONE){
                    $htmlOptions['requiredOne'] = true;                     
                };
                if($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_ENABLED_CURRENT){
                    $visibleLanguages = $editLanguage; 
                };
                if($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_MANDATORY_CURRENT){
                    $htmlOptions['required'] = true;
                    $visibleLanguages = $editLanguage; 
                };
            ...
            <?php if ($attribute_type == FieldType::TEXT) { 
                    ...
                    if ($attribute == 'title') { ?>
                    <?php echo $form->hiddenFields($participant, 'title', $conf->getLanguages()); ?>    
                <?php } else { ?>
            <div class="row" >
                <?php echo $form->label($participant, $attribute, $htmlOptions);?><br />
                <?php if(!empty($attribute_hint)){ ?><span class="hint"><?php echo StringUtils::prepareHtml($attribute_hint);?></span><?php };?>
                <?php echo $form->textAreas($participant, $attribute, array('cols' => 97, 'rows' => 2, 'class' => 'editor value', 'tableClass' => 'value', 'hintClass' => 'editor-hint'), 'vertical', $conf->getLanguages(), $visibleLanguages );?>
            </div>
                <?php } ?>
            <?php } ?>
            <!-- конец поле-текст -->
        ...
        <div class="row authors-row" >
            ...
            ?> > 
            <a id="authorName<?php echo $i ?>" href="javascript:void(0)" class="author-link" onclick="selectAuthor(<?php echo $i ?>);" ><?php echo $author->authorName(); ?></a>
            &nbsp;
            ...
            <div id="authorDiv<?php echo $i; ?>" class="<?php if($i!==0){ echo 'author-hidden';}else {echo 'author-shown';}; ?>" >
                ...
                <?php foreach($aAttributesEnabled as $k => $attribute) { 
                    $htmlOptions = array();
                    $visibleLanguages = NULL;
                    if ($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_DISABLED) {
                        continue; // пропускаем поле
                    };
                    if ($settings->{$attribute . '_wo_paper_mode'} == AppFormSettings::MODE_MANDATORY_ALL) {
                        $htmlOptions['required'] = true;
